    ...
    wchar_t const *aSingleFilterDescription) /* NULL or "image files" */
{
  ...
  if (quoteDetectedW(aDefaultPathAndFile))
    return tinyfd_saveFileDialogW(aTitle, L"INVALID DEFAULT_PATH WITH QUOTES",
                                  aNumOfFilterPatterns, aFilterPatterns,
                                  aSingleFilterDescription);
  if (quoteDetectedW(aSingleFilterDescription))
    return tinyfd_saveFileDialogW(aTitle, aDefaultPathAndFile,
                                  aNumOfFilterPatterns, aFilterPatterns,
                                  ...
                                  return */
{
  ...
  if (quoteDetectedW(aTitle))
    return tinyfd_openFileDialogW(
        L"INVALID TITLE WITH QUOTES", aDefaultPathAndFile, aNumOfFilterPatterns,
                     ...
                     unsigned char aoResultRGB[3])       /* { 0 , 0 , 0 } */
{
  ...
  if (quoteDetectedW(aTitle))
    return tinyfd_colorChooserW(L"INVALID TITLE WITH QUOTES", aDefaultHexRGB,
                                aDefaultRGB, aoResultRGB);
  if (quoteDetectedW(aDefaultHexRGB))
    return tinyfd_colorChooserW(aTitle, L"INVALID DEFAULT_HEX_RGB WITH QUOTES",
                                aDefaultRGB, aoResultRGB);
    ...
    int aAllowMultipleSelects)            /* 0 or 1 */
{
  ...
  if (tfd_quoteDetected(aTitle))
    return tinyfd_openFileDialog(
        "INVALID TITLE WITH QUOTES", aDefaultPathAndFile, aNumOfFilterPatterns,
        aFilterPatterns, aSingleFilterDescription, aAllowMultipleSelects);
  if (tfd_quoteDetected(aDefaultPathAndFile))
    return tinyfd_openFileDialog(
                    ...
                    unsigned char aoResultRGB[3])       /* { 0 , 0 , 0 } */
{
  ...
  if (tfd_quoteDetected(aDefaultHexRGB))
    return tinyfd_colorChooser(aTitle, "INVALID DEFAULT_HEX_RGB WITH QUOTES",
                               aDefaultRGB, aoResultRGB);
...
void tinyfd_beep(void) {
  ...
  if (osascriptPresent()) {
    ...
    /*strcpy( lDialogString , "pactl load-module module-sine frequency=440;sleep
     * .3;pactl unload-module module-sine" ) ;*/
     ...
     * --test sine > /dev/tty" ) ;*/
    strcpy(lDialogString,
           "( speaker-test -t sine -f 440 > /dev/tty )& pid=$!;sleep .4; kill "
